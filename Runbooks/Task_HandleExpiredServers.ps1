...
"@ -f $server.Name
            ...
            $pendingrequest = $requestresults | Where-Object {($_.Status -eq 2) -and ($_.Entitystate -eq 0) -and ($_.StartDate -le $datetime) } | Sort-Object RequestNumber            
            if ($pendingrequest) {
                Write-Output "There are pending requests, selecting the first one"
                $nextrequest = $pendingrequest | Select-Object -First 1
